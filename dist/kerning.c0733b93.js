...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/kerning.js":[function(require,module,exports) {
...
(function ($) {
  ...
  (function () {
    ...
    var hasSameOrigin = function hasSameOrigin(url) {
      return url === url.replace(/^([^\/]*)\/\/([^@]*@)?([^\/:]+)(:\d+)?.*/, function (match, protocol, user, hostname, port, offset, string) {
        ...
        if (protocol !== window.location.protocol) return ""; // "protocol mismatch: " + protocol + " vs. " + window.location.protocol

        if (hostname !== window.location.hostname) return ""; // "hostname mismatch: " + hostname + " vs. " + window.location.hostname

        if (port !== window.location.port) return ""; // "port mismatch: " + port + " vs. " + window.location.port
    ...
    $.fn.parsecss = function (callback, parseAttributes) {
      ...
      this.findandfilter('style').each(function () {
        ...
        if (!this.disabled && hasSameOrigin(this.href) && $.parsecss.mediumApplies(this.media)) $.get(this.href, parse);
      }).end();
    ...
    $.parsecss = function (str, callback) {
      ...
      $.each(str.split('`b%'), function (i, css) {
    ...
    $.parsecss.isValidSelector = function (str) {
      var s = $('<style>' + str + '{}</style>').appendTo('head')[0]; // s.styleSheet is IE; it accepts illegal selectors but converts them to UNKNOWN. Standards-based (s.shee.cssRules) just reject the rule
    ...
    $.parsecss.styleAttributes = styleAttributes; // caches
    ...
    $.each(['show', 'hide'], function () {
      ...
      $.fn[plugin] = function () {
        ...
        if ($.fn[name]) {
          ...
          if (_typeof(args[1]) != 'object') args.splice(1, 0, {});
          fn = _show[which];
        } else {
          // regular show/hide
          fn = _show[which];
    ...
    function styleAttributes(HTMLtext, callback) {
      var ret = '',
          style,
          tags = {}; //  keep track of tags so we can identify elements unambiguously
  ...
  var unstack = function () {
    var fontunstack = {
      ...
      analyzeStack: function analyzeStack(stack, elems) {
        var generics = ["monospace", "sans-serif", "serif", "cursive", "fantasy"];
        var baseline = generics[0];
        var num_fonts = stack.length;
        var last_resort = stack[num_fonts - 1]; // If author hasn't included a generic (tsk, tsk), let's add one
      ...
      testFont: function testFont(requested_font, baseline_font) {
        ...
        span.remove(); // If the dimensions change, the font is installed
  ...
  window.Kerning = new function () {
    ...
    var self = this,
        ...
        browserPrefix = ['webkitTransform' in document.documentElement.style && 'webkit', navigator.userAgent.indexOf("MSIE") > -1 && 'ms', "MozTransform" in document.documentElement.style && 'moz', window.opera && 'o'].reduce(function (pv, cv) {
      return pv + (cv || '');
    }),
        osPrefix = [nav.match(/Mac/) && 'mac', nav.match(/Win/) && 'win', nav.match(/Linux/) && 'linux'].reduce(function (pv, cv) {
      return pv + (cv || '');
    });
    var methods = {
      // Match -[letter|word]-pairs(…) values
      _pairs: function _pairs(type, elements, pairString) {
        ...
        var els = type === 'word' ? elements.children('span') // for -word-pairs
        : elements.find('span > span') // for -letter-pairs
        // we parse the string slightly differently if a transform is used
        ,
            isTransform = pairString.match(/translate|rotate|skew|perspective/i) // matches and splits the pairing rules
        ,
        ...
        $.each(pairs, function (index, pair) {
          pairInfo = pair.split(':'); // match the content inside the pair (stripping the leading and tailing quotes)
          ...
          if (type === 'word') pairKeys = pairInfo[0].split(' ');else pairKeys = pairInfo[0];

          pairDown = function pairDown(index) {
            ...
            if (pairKeys[1] !== ' ') {
              ...
              nextWord = type == 'word' ? $(this).next('[class^="word"]') // if one of the pairKeys is just a space and we're doing letter pairs,
      ...
      _repeats: function _repeats(type, elements, repeatString) {
        var usingRepeats = repeatString.match(/^-(letter|word)-repeats\(([\s\S]+)\)$/i);
        if (!usingRepeats || usingRepeats[1] !== type) return false;
        var els = type === 'word' ? elements.children('span') : elements.find('span > span'),
        ...
        $.each(repeats, function (index, repeat) {
          ...
          if (isTransform && repeatInfo[1].substring(repeatInfo[1].length - 1) !== ')') repeatInfo[1] += ')';
      ...
      _conditional: function _conditional(type, elements, rule) {
        var usingConditional = rule.match(/^(?:-(letter|word)-)?if-font\(([\s\S]+)\)$/i);
        if (!usingConditional) return;
        var els = type === 'all' ? elements : type === 'word' ? elements.children('span') : elements.find('span > span'),
        ...
        elements.each(function (i, el) {
          ...
          if (!fontElements[fontInUse]) fontElements[fontInUse] = [el];else fontElements[fontInUse].push(el);
        });
        $.each(fonts, function (index, font) {
          fontInfo = font.match(/['"]([^'"]+)['"]:\s*(.+)$/);
          if (!fontInfo) return true;
          fontInfo = fontInfo.splice(1);
          if (fontInfo[0] in fontElements) elementSet.push([$.trim(fontInfo[1]), $(fontElements[fontInfo[0]])]);
        });
        return elementSet;
      },
      // Parse and apply a CSS property
      _applyAttribute: function _applyAttribute(type, elements, attribute, values) {
        var conditional = methods._conditional(type, elements, values);

        if (!conditional || !conditional.length) conditional = [[values, elements]];
        $.each(conditional, function (a, ve) {
          var vals = ve[0],
              els = ve[1];

          var custom = methods._pairs(type, els, vals) || methods._repeats(type, els, vals);

          if (custom) {
            $.each(custom, function (index, valEl) {
              if (typeof attribute !== 'string') {
                var attrs = {};
                $.each(attribute, function (a, attr) {
                  attrs[attr] = valEl[0];
                });
                valEl[1].css(attrs);
              } else {
            ...
            var indexValues, keys, transformGroups; // check for transform groups, as we need to parse these slightly differently
            ...
            els.each(function (i, el) {
              keys = type === 'all' ? $(el) // match the entire word (only used for certain use cases)
              : type === 'word' ? $(el).children('span') : $(el).find('span > span'); // letters are spans inside words
      ...
      kern: function kern(type, elements, kerning) {
        methods._applyAttribute(type, elements, 'margin-right', kerning);
      },
      size: function size(type, elements, sizes) {
        methods._applyAttribute(type, elements, 'font-size', sizes);
      },
      weight: function weight(type, elements, weights) {
        methods._applyAttribute(type, elements, 'font-weight', weights);
      },
      color: function color(type, elements, colors) {
        methods._applyAttribute(type, elements, 'color', colors);
      },
      backgroundcolor: function backgroundcolor(type, elements, colors) {
        methods._applyAttribute(type, elements, 'background-color', colors);
      },
      transform: function transform(type, elements, transforms) {
        var attributes = ['-webkit-transform', '-moz-transform', '-ms-transform', '-o-transform', 'transform'];
    ...
    this._parse = function (css, ignoreParsed) {
      ...
      for (var selector in css) {
        for (var property in css[selector]) {
          var match,
              elements,
              value = css[selector][property]; // Kerning.js prefixed selectors
          ...
          if (match = property.match(new RegExp('^(-' + browserPrefix + '|-' + osPrefix + ')?-(letter|word)-(kern|transform|size|color|backgroundcolor|weight)', 'i'))) {
            ...
            elements = $(selector);
            if (ignoreParsed) elements = elements.not('.kerningjs');
            elements.not('.kerningjs').addClass('kerningjs').css('visibility', 'inherit').lettering('words').children('span').css('display', 'inline-block') // break down into words
            .lettering().children('span').css('display', 'inline-block'); // break down into letters

            if (methods[action]) methods[action].call(this, specificity, elements, value); // existing selectors with Kerning.js-custom values
          } else if ((match = property.match(/font-(size|weight)/i)) && value.match(/if-font/i)) {
            var action = match[1].toLowerCase();
            elements = $(selector);
            if (ignoreParsed) elements = elements.not('.kerningjs');
            elements.not('.kerningjs').addClass('kerningjs').css('visibility', 'inherit');
            if (methods[action]) methods[action].call(this, 'all', elements, value);
          }
        }
      }
    };
    ...
    this.refresh = function (newElementsOnly) {
      if (self._parsedCSS) self._parse(self._parsedCSS, newElementsOnly);
...
},{}]},{},["../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/kerning.js"], null)
//# sourceMappingURL=/kerning.c0733b93.map